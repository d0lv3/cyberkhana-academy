...
const ProgrammingModuleEditor: React.FC = () => {
   ...
   * The client only checks the stash; the server authorises the write itself,
   * by role or by grant. */
  const isAdminEdit =
    searchParams.get('admin') === '1' || searchParams.get('shared') === '1';
  ...
  return (
    ...
    >
      ...
      {adminCtx && (
        <div
          className={`flex items-start gap-3 rounded-lg border px-4 py-3 mb-4 max-w-2xl ${
        ...
        >
          ...
          <div className="text-xs text-[#d2d7e3]">
            ...
            you're editing{' '}
            <span className="font-semibold text-[#f3f6ff]">{adminCtx.ownerName}</span>'s module.
